...
def EXPLoRaTS(ts_nodes, bsid, t):
    ...
    for nd in range(0, len(ts_nodes)):
        if ts_nodes[nd].minBS == bsid:
            MP = float(ts_nodes[nd].period)
            PL = float(ts_nodes[nd].plen)
            SF = float(ts_nodes[nd].sf)
            N_mess = T / MP
            ...
            ts_nodes[nd].symboltime = N_sym_usr
            print "Node ID:", ts_nodes[nd].id, "Symbol Time:", N_sym_usr, "PLEN:", PL
    ...
    for i in range(0, len(ts_nodes)):
        if ts_nodes[i].minBS == bsid:
            bucket_nodes[ts_nodes[i].sf - 7] += 1

    print "test", bsid, ":", bucket_nodes
...
if experiment == 2:
    ...
    for bsid in range(0, nrBS):
        ...
        for nd in range(0, len(nodes)):
            if nodes[nd].minBS == bsid:
                SF_vec[nodes[nd].sf - 7] += 1

        print "BS", bsid, ":", SF_vec
            ...
            if sf_temp[i] > new_sf_list[i]:
                ...
                for num in range(0, count):
                    ...
                    for j in range(0, len(nodes)):
                        if nodes[j].minBS == bsid:
                            if nodes[j].sf == i + 7:
                                if nodes[j].dist[bsid] >= max_dist:
                                    max_dist = nodes[j].dist[bsid]
                                    max_dist_id = j


                    nodes[max_dist_id].sf = nodes[max_dist_id].sf + 1
                    for bs_tmp in range(0, nrBS):
                        nodes[max_dist_id].packet[bs_tmp].sf = nodes[max_dist_id].packet[bs_tmp].sf + 1
...
if experiment == 3:
    ...
    for nd in range(0, len(nodes)):
        ...
        global sensi_125
        for bs_tmp in range(0, nrBS):
            nodes[nd].packet[bs_tmp].sf = nodes[nd].sf
            nodes[nd].packet[bs_tmp].symTime = (2.0 ** nodes[nd].packet[bs_tmp].sf) / nodes[nd].packet[bs_tmp].bw
            nodes[nd].packet[bs_tmp].lost = nodes[nd].packet[bs_tmp].rssi < sensi_125[nodes[nd].packet[bs_tmp].sf - 7]
        ...
        if bs_rx_count > 1:
            nodes[nd].overlap = True
        #print "node {} bs {} lost {}".format(self.nodeid, self.bs, self.lost)
        env.process(transmit(env, nodes[nd]))
...
# Trace File
fname = "/home/lab-3714/LoRaSim/Output/Method1/exp" + str(experiment) + "d99" + "BS" + str(nrBS) + "Intf.csv"
#print "Trace File:dr", fname, ", basestation.txt, nodes.txt, nodes.jpg"
totalnodes = nrNodes * nrBS
if os.path.isfile(fname):
    csv = "\n" + str(totalnodes) + "," + str(nrBS) + "," + str(experiment) + "," + str(p_value) + "," + str(baseDist) + "," + str(avgSendTime) \
          + "," + str(simtime) + "," + str(derALL) + "," + str(sf_d[0]) + "," + str(sf_d[1]) + "," + str(sf_d[2]) + "," + str(sf_d[3]) + "," + str(sf_d[4]) + "," + str(sf_d[5])
else:
...
if p_value == 0.3:
    ...
    plt.legend(loc=0)
    picturename = "/home/lab-3714/LoRaSim/Output/Method1/Figure/Exp-" + str(experiment) + "-nodes-loc-" + str(time.strftime("%Y%m%d-%H%M%S", time.localtime())) + ".png"
    ...
    plt.legend(loc=0)
    picturename2 = "/home/lab-3714/LoRaSim/Output/Method1/Figure/Exp-" + str(experiment) + "-nodes-der-" + str(time.strftime("%Y%m%d-%H%M%S", time.localtime())) + ".png"
