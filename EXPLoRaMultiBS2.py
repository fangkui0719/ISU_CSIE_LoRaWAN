...
if experiment == 2:
    ...
    for bsid in range(0, nrBS):
        ...
        for sf_i in range(0, 6):
            sf_check = sf_check + sf_temp[sf_i]
            exp_cnt = exp_cnt + new_sf_list[sf_i]
                ...
                for num in range(0, count):
                    ...
                    for j in range(0, len(nodes)):
                        if nodes[j].bs_rx[bsid] == 1:
                            if nodes[j].sf == i + 7:
                                #check_dist = nodes[j].dist[bsid]
                                #print "node", j, "bs:", bsid, "distance: ", check_dist
                                ...
                                if min_dist == 99999:
                                    ...
                                    if nodes[j].overlap:
                                        ...
                                        if nodes[min_dist_id].overlap == False:
                                            if check_dist <= min_dist:
                                                min_dist = check_dist
                                                min_dist_id = j
...
for i in range(0, len(packetsRecED)):
    ...
    print len(packetsRecED[i]), nodes[i].sent, nodes[i].der

# store nodes and basestation locationsEXPLoRa
with open('nodes.txt', 'w') as nfile:
    for node in nodes:
        nfile.write('{id} {x} {y} {sf} {type} {der} {overlap}\n'.format(**vars(node)))
...
# Trace File
fname = "/home/lab-3714/LoRaSim/Output/Method2/exp" + str(experiment) + "d99" + "BS" + str(nrBS) + "Intf.csv"
...
if p_value == 0.3:
    ...
    plt.legend(loc=0)
    picturename = "/home/lab-3714/LoRaSim/Output/Method2/Figure/Exp-" + str(experiment) + "-nodes-loc-" + str(time.strftime("%Y%m%d-%H%M%S", time.localtime())) + ".png"
    ...
    plt.legend(loc=0)
    picturename2 = "/home/lab-3714/LoRaSim/Output/Method2/Figure/Exp-" + str(experiment) + "-nodes-der-" + str(time.strftime("%Y%m%d-%H%M%S", time.localtime())) + ".png"
